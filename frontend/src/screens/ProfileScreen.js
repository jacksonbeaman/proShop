...
const ProfileScreen = ({ location, history }) => {
  ...
  // we the userInfo from state.userLogin in order to get the token
  ...
  useEffect(() => {
    if (!userInfo) {
      ...
      if (!user || !user.name || success) {
        ...
        // set local component level state to populate the form field with logged in user info
        setName(user.name);
        setEmail(user.email);
      }
    }
            ...
            <tbody>
              {orders.map((order) => (
                <tr key={order._id}>
                  ...
                  {/* substring(0,10) - gives us first 10 chars */}
